 ...
 */
export function getRandomFashionImage(category: string, seed?: string): string {
  ...
  let index;
  if (seed) {
    // A simple hash function to get a deterministic index from the seed
    let hash = 0;
    ...
    index = Math.abs(hash) % categoryImages.length;
  } else {
    index = Math.floor(Math.random() * categoryImages.length);
  }
  const selectedImage = categoryImages[index];

  return generateUnsplashUrl(selectedImage);
}
