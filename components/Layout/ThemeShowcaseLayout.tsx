...
import {
  ProductCardVariant as CardV,
  FOOTER_VARIANTS,
  FooterVariant as FooterV,
  NAVBAR_VARIANTS,
  NavbarVariant as NavbarV,
...
function withParams(
  ...
  searchParams: ThemeShowcaseLayoutProps["searchParams"]
) {
        ...
        <div className="w-full px-4 py-4 flex flex-col gap-3">
          ...
          <div>
            ...
            {NAVBAR_VARIANTS.map((v) => (
              <Link
                key={`nav-${v}`}
                href={withParams(basePath, { navbar: v }, searchParams)}
                className={`btn btn-xs mr-2 ${
                  v === navbar ? "btn-primary" : "btn-ghost"
                }`}
              >
                V{v}
              </Link>
          ...
          <div>
            ...
            {FOOTER_VARIANTS.map((v) => (
              <Link
                key={`foot-${v}`}
                href={withParams(basePath, { footer: v }, searchParams)}
                className={`btn btn-xs mr-2 ${
                  v === footer ? "btn-primary" : "btn-ghost"
                }`}
              >
                V{v}
              </Link>
          ...
          <div>
            ...
            {PRODUCT_CARD_VARIANTS.map((v) => (
              <Link
                key={`card-${v}`}
                href={withParams(basePath, { card: v }, searchParams)}
                className={`btn btn-xs mr-2 ${
                  v === productCard ? "btn-primary" : "btn-ghost"
                }`}
              >
                V{v}
              </Link>
        ...
        <div className="hero-content text-center">
          <div>
            <h1 className="text-4xl font-bold">
              {category.toUpperCase()} Theme Showcase
            </h1>
            <p className="opacity-90">
              Select variants above to see layout changes.
            </p>
          </div>
        </div>
      </div>
