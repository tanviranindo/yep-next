...
import { Shield, Lock, Truck, Award, ChevronLeft, ChevronRight, Leaf } from "lucide-react";

export type BeautyHeroVariant = 1 | 2;

// Variant 1 Props (Glamour Beauty)
...
// Variant 2 Props (NIKA Natural Beauty)
...
/**
 * BeautyHero - Unified component for Beauty hero variants
 *
 * Variant 1: Glamour Beauty hero with model image and features
 * Variant 2: NIKA Natural Beauty hero with carousel slider
 */
export default function BeautyHero(props: BeautyHeroProps) {
  const [currentSlide, setCurrentSlide] = useState(0);

  if (props.variant === 2) {
    // Variant 2: NIKA Natural Beauty Hero with Carousel
    const {
      slides = [
        {
  ...
  const {
    ...
    features = [
      { icon: "certified" as const, label: "Certified Products" },
      { icon: "secure" as const, label: "Secure Payment" },
      { icon: "shipping" as const, label: "Fast Shipping" },
      { icon: "transparent" as const, label: "100% Transparent" },
    ],
  } = props;
