...
/**
 * GadgetProductCard - Unified component for Gadget product card variants
 *
 * Variant 1: Gadget Store product card with ProductCardVariant4
 * Variant 2: TechHub product card with hover effects
 */
export default function GadgetProductCard({ variant, product, className }: GadgetProductCardProps) {
  ...
  const handleAddToCart = (e?: React.MouseEvent) => {
    ...
    addToCart({
      ...
      name: product.name,
      price: product.price,
      image: product.image || (variant === 1 ? "/items/gadget1.jpg" : "/items/gadget2.jpg"),
    });
  };
  ...
  if (variant === 2) {
    // Variant 2: Gadget2 / TechHub Product Card
        ...
        <div className="relative">
          ...
          <div className="relative w-full aspect-square bg-gray-100 overflow-hidden">
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              ...
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-sm ${
                  isInWishlist(product.id)
                    ...
                    : "bg-white/90 hover:bg-blue-600 text-gray-700 hover:text-white"
                }`}
              >
                <svg
                  className="w-4 h-4"
              ...
              {/* Quick View Icon */}
              <Link href={`/gadget2/product/${product.id}`}>
                <button className="w-8 h-8 rounded-full bg-white/90 flex items-center justify-center hover:bg-blue-600 text-gray-700 hover:text-white transition-colors shadow-sm">
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
            ...
            <div className="absolute bottom-0 left-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <button
                ...
                className="w-full bg-blue-600 text-white py-3 font-semibold text-sm uppercase tracking-wider hover:bg-blue-700 transition-colors"
              >
                ADD TO CART
              </button>
            </div>
          ...
          <div className="mt-4 space-y-2">
            <h3
              ...
              itemProp="name"
            >
              <Link href={`/gadget2/product/${product.id}`}>{product.name}</Link>
            </h3>
  ...
  // Variant 1: Gadget Store Product Card
  const actions = [
    {
      icon: (
        <svg
